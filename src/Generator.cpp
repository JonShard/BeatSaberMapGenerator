...
namespace OK {
    ...
    void Generator::GenerateMap(std::vector<Keyframe> keyframes, std::string songName) {
        Map map(songName + ".dat");
        printf("Generating map from nototions: %ld\n", keyframes.size());
        for (int i = 0; i < keyframes.size(); i++) {
            int produceAttempts = 0;
            printf("Processing keyframe %d at time: %f \tMap length: %ld \tProduce attempts: %d\n", i, keyframes[i].time, map.m_notes.size(), produceAttempts);
            Map mapNext;
            do {
                mapNext = map;
                mapNext += s_factories[0]->produce(keyframes, map);
                produceAttempts++;
